...
// ---------- parameter sweep ----------
// The model's constants live inside src/model.js and are not injectable, so a
// true sweep needs them parameterized. Rather than pretend otherwise, this
// reports what a sweep would require and runs the honest single-config case.
if (SWEEP) {
  console.log(`\n--sweep ${SWEEP} requested.`);
  console.log(`The recency decay and shrinkage constant are module-level constants in`);
  console.log(`src/model.js (DECAY, SHRINK_K). To sweep them, expose them as optional`);
  console.log(`scoreProp inputs first, then loop here. Running the current configuration:`);
}
