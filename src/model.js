...
function priorCv(propType, kind, override) {
  if (override !== undefined && override !== null) return override;
  const t = String(propType || "").toLowerCase();
  if (/pass/.test(t) && /yd|yard/.test(t)) return 0.25;
  if (/rush/.test(t) && /yd|yard/.test(t)) return 0.45;
  // Was 0.50, a judgment call never checked against real outcomes. A sweep
  // against 19,787 real cases (6 seasons, 2021-2026, via the backtest
  // harness) found central-50 coverage at 0.50 was 38.8% against a 50%
  // target, meaning intervals were badly too narrow, and tested values from
  // 0.20 to 1.20 in steps of 0.10. 1.1 was the best fit, landing at 50.9%
  // coverage with the KS statistic dropping from 0.1353 to 0.1085. This
  // also incidentally fixed a by-week shape problem: coverage under the old
  // value climbed steadily across a season (33% by week 4 up to 42% by week
  // 18, never closing the gap); under 1.1 it holds steady in the 48-55%
  // range nearly the entire season. Receiving yardage is genuinely
  // higher-variance than the old prior assumed, a player can produce 0 in
  // one game and 150 the next, and the fixed prior was not reflecting that.
  if (/rec/.test(t) && /yd|yard/.test(t)) return 1.1;
...
// mean estimate would.
//
// KNOWN UNRESOLVED LIMITATION, confirmed against real data, not yet fixed:
// a backtest sweep of this constant (1 through 50) against 13,343 real
// rushing cases and 2,933 real passing cases (6 seasons, 2021-2026) found
// that no single value corrects the model's calibration shape across a
// season. Under the current default, coverage starts too low early in a
// season (26-36% around week 4-5, overconfident, too little data banked
// yet) and drifts past the 50% target later in the season (55-70% by weeks
// 19-21, underconfident, more real variance banked than the shrinkage still
// accounts for). Tightening SHRINK_K helps the early weeks and worsens the
// late ones; loosening it does the reverse, the KS statistic barely moved
// across the whole swept range (0.1856 rushing, 0.12-0.15 passing) because
// a single constant can only shift the whole curve, not correct a curve
// that bends the wrong direction partway through. The real fix is a
// schedule where the shrinkage weight itself changes as games accumulate
// within a season, not a different fixed number, that is a genuine model
// change, not implemented here yet. Run
// `node backtest/backtest.mjs --data <historical> --stat rushing_yards
// --sweep shrinkK` to reproduce.
const SHRINK_K = 5;
function shrinkVariance(observedVar, priorVar, n, kOverride) {
  const k = kOverride !== undefined && kOverride !== null ? kOverride : SHRINK_K;
  if (!(n > 1)) return priorVar;
  return (n * observedVar + k * priorVar) / (n + k);
}
...
function scoreProp(p) {
  ...
  // Preferred: exponentially weighted game log (needs >=2 entries to compute
  // both a mean and an observed variance). Fallback: the v8-style blend of
  // last-3 and season average, retained so existing saved props still score.
  const gameLog = parseGameLog(p.gameLog);
  let baseMean, observedVar = null, logN = 0, singleGameMean = false;

  // MEAN_SHRINK_K: how many "pseudo-games" of the stabilizing prior a single
  // real game is weighed against. A lone game, however extreme, is real
  // signal and shouldn't be discarded, but early in a season it also
  // shouldn't move the projection all the way to that one number, so it's
  // blended rather than taken at face value.
  const MEAN_SHRINK_K = 3;
  ...
  if (gameLog.length >= 2) {
    ...
    logN = gameLog.length;
  } else if (gameLog.length === 1) {
    // Exactly one real game. This case previously fell through untouched to
    // the "no player data" branch below whenever last3/seasonAvg were not
    // also filled in, silently discarding the one real number entered. Early
    // in a season, every player is in exactly this position (one game
    // played), so this was not a rare edge case, it was the default.
    //
    // The single game is real signal and is used, blended toward a
    // stabilizing prior (the prior-year baseline if given, otherwise the
    // market line, which is itself an efficient, well-informed estimate)
    // rather than either discarded or taken at 100% face value. A single
    // monster or disaster game should nudge the projection, not define it.
    const singleGame = gameLog[0];
    const priorYrBaselineEarly = isNum(p.priorYrBaseline) ? Number(p.priorYrBaseline) : null;
    const stabilizingPrior = priorYrBaselineEarly !== null ? priorYrBaselineEarly : line;
    baseMean = isNum(stabilizingPrior)
      ? (singleGame + MEAN_SHRINK_K * stabilizingPrior) / (1 + MEAN_SHRINK_K)
      : singleGame;
    logN = 1;
    singleGameMean = true;
  ...
  // Usage trend: data-driven when WOPR is supplied, subjective slider
  // otherwise. WOPR (weighted opportunity rating = 1.5 x target share + 0.7 x
  // air yards share) is a published composite opportunity metric that already
  // combines target share and air yards share into one number, so it is used
  // instead of either individually to avoid counting the same signal twice.
  // Only applies to receiving props, since WOPR is a receiving-usage metric;
  // rushing and passing props fall back to the subjective slider.
  // Capped at the same +/-7% as before, and still intended only for
  // information not yet visible in the game log, a real WOPR swing between
  // a stated baseline and recent role is exactly that kind of information.
  const isReceivingProp = /rec|reception|target/i.test(p.propType || "");
  let usageAdj, usageAdjSource;
  if (isReceivingProp && isNum(p.recentWopr) && isNum(p.baselineWopr) && Number(p.baselineWopr) > 0) {
    const dataUsageTrend = (Number(p.recentWopr) - Number(p.baselineWopr)) / Number(p.baselineWopr);
    usageAdj = clamp(dataUsageTrend, -0.07, 0.07);
    usageAdjSource = "WOPR (data-driven)";
  } else {
    usageAdj = clamp(num(p.usageTrend, 0), -0.07, 0.07);
    usageAdjSource = "manual slider";
  }

  // Efficiency check from EPA per opportunity, layered onto the role
  // adjustment rather than replacing it, since a single week of EPA is noisy
  // and should temper a subjective role score rather than override it
  // outright. Distinguishes a player winning more with the same volume from
  // one who is not. League-average EPA per play/target sits near 0.0;
  // +/-0.30 is roughly the range from a clearly negative to clearly positive
  // efficiency week. Capped at a modest +/-6%, smaller than the matchup or
  // role caps, because one week of EPA carries less information than a full
  // game log or a real matchup fact.
  const epaField = isRush ? "rushingEpa" : isReceivingProp ? "receivingEpa" : "passingEpa";
  let efficiencyAdj = 0;
  if (isNum(p[epaField])) {
    efficiencyAdj = clamp((Number(p[epaField]) / 0.30) * 0.06, -0.06, 0.06);
  }
  ...
  const totalAdj = clamp(
    matchupAdj + weatherAdj + roleAdj + usageAdj + efficiencyAdj + pressureAdj + redZoneAdj,
    -0.5, 0.5
  );
  ...
  // __cvOverride and __shrinkKOverride are for the backtest sweep only
  // (backtest/backtest.mjs), never set by the live app, so normal scoring is
  // completely unaffected by their presence in this code.
  const cv = priorCv(p.propType, kind, p.__cvOverride);
  ...
  if (isNum(p.stdDev)) {
    ...
    variance = shrinkVariance(scaledObserved, priorVar, logN, p.__shrinkKOverride);
    varianceSource = `game log (n=${logN}, shrunk toward prior`;
    varianceSource += `, CV prior ${cv})`;
  } else if (singleGameMean) {
    // One real game gives a point estimate but no way to compute a sample
    // variance from it. Predicting a new game from an estimated mean carries
    // two sources of uncertainty that both need to be counted: the spread of
    // outcomes around the true mean (the prior variance), and how uncertain
    // that mean estimate itself is from only one observation. The standard
    // result for a prediction interval built from a sample mean of n draws
    // is population variance times (1 + 1/n); at n=1 that doubles the width
    // versus treating the mean as already known, and it relaxes toward the
    // plain prior as n grows. This was the missing piece that let one
    // outlier game (in either direction) produce an overconfident-looking
    // probability: the projection moved to reflect the single game, but the
    // uncertainty band around it did not widen to match.
    variance = priorVar * (1 + 1 / logN);
    varianceSource = `single game (n=1, prior variance widened ${'\u00d7'}2 for small-sample mean uncertainty, CV prior ${cv})`;
  } else {
    variance = priorVar;
  ...
  // A projection resting entirely on the market line, with zero real player
  // data behind it, carries no independent information and should never
  // present as a confident edge. This must check for genuinely zero game log
  // entries, not "fewer than 2": a single real game (the singleGameMean case
  // above) is real signal, already reflected in a wider variance and, via
  // effectiveN above, an appropriate cap, and should not be blanket-forced to
  // Low on top of that, which would double-penalize the same small sample.
  if (gameLog.length === 0 && !isNum(p.last3) && !isNum(p.seasonAvg)) confidence = "Low";
  ...
  return {
    ...
    usageAdjSource,
    efficiencyAdj: round3(efficiencyAdj),
